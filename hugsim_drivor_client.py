...
def main(args):
    ...
    while True:
        ...
        print(f'[Frame {cnt}] Command Index : {info["command"]}, Command Vector: {command}')
        with open(os.path.join(args.output, "drivor_command_debug.txt"), "a") as f: f.write(f"frame={cnt}, command={info['command']}, accelerate={info['accelerate']}, ego_steer={info['ego_steer']}, ego_velo={info['ego_velo']}, timestamp={info['timestamp']}\n")
